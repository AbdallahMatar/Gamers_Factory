...
@section('content')
    <div class="container mt-5 items-center p-5 box border-b border-gray-200">
        @if ($errors->any())
            ...
            @endforeach
        @endif
        <form action="{{ route('states.update',$state->id) }}" method="POST" id="create_service_form" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div>
                <label>City</label>
                <select name="city_id" data-hide-search="true" class="select2 w-full">
                    <option>--Select--</option>
                    @foreach ($cities as $city)
                        <option value="{{ $city->id }}" @if ($state->city_id == $city->id) selected @endif>{{ $city->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label>Name</label>
                <input type="text" name="name" value="{{ $state->name }}" class="input w-full border mt-2"
                    placeholder="Name">
            </div>
            <div class="text-right mt-3">
                <div class="w-full sm:w-auto flex items-center sm:ml-auto mt-3 sm:mt-0">
                    ...
                    <input name="status" data-target="#basic-textual-toast" class="show-code input input--switch border"
                        type="checkbox" @if ($state->status === 'Active') checked @endif>
                </div>
            </div>
            <div class="text-right mt-3">
